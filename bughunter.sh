...
combine_and_send_results() {
  # Pastikan semua file yang dibutuhkan tersedia
  if [[ ! -s "$subs_file" && ! -s "$alive_file" && ! -s "$merged_params" && ! -s "$xss_file" && ! $(find reports -type f -name '*.txt' | grep -q .) ]]; then
    echo "[!] Tidak ada data tersedia untuk dikompilasi. Jalankan langkah 2–5 terlebih dahulu."
    send_telegram "⚠️ Gagal menggabungkan laporan: tidak ada data hasil scan tersedia!"
    return
  fi

  local final_output="final-report.txt"
  local domain=$(head -n 1 "$subs_file" | cut -d'.' -f2- 2>/dev/null)
